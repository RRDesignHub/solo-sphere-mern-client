...
const UpdateJob = () => {
  ...
  const fetchAllJobs = async () => {
    const { data } = await axios.get(
      `${import.meta.env.VITE_API}/job/${id}`
    );
    setJob(data);
    setStartDate(new Date(data?.deadline))
  };
  const {jobTitle, buyer, deadline, jobCategory, minPrice, maxPrice, jobDescription, bid_count} = job;
  ...
  const handleUpdateJob = e =>{
    ...
    try{
      axios.put(`${import.meta.env.VITE_API}/updateJob/${id}`, formData)
        .then(res => {
      ...
      <section className=' p-2 md:p-6 mx-auto bg-white rounded-md shadow-md '>
        ...
        <form onSubmit={handleUpdateJob}>
          <div className='grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2'>
            ...
            {
              jobCategory && <div className='flex flex-col gap-2 '>
